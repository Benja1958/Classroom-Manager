...
class TestRoutes(unittest.TestCase):
    ...
    def test_create_course(self):
        ...
        self.assertEqual(
            ...
            list,
            wrong_value_error(
                req_type, route, type(course.get(
                    "assignments")), list, "type of assignments field"
            ),
        )
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, type(course.get(
                    "instructors")), list, "type of instructors field"
            ),
        )
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, type(course.get(
                    "students")), list, "type of students field"
            ),
        )
    ...
    def test_create_bad_course(self):
        ...
        self.assertIsNotNone(res.json()["error"], wrong_value_error(
            req_type, route, res.json()["error"], str, "error", "type of error field"))
    ...
    def test_create_user(self):
        ...
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, type(
                    user.get("courses")), list, "type of courses field"
            ),
        )
    ...
    def test_add_student_to_course(self):
        ...
        self.assertEqual(
            ...
            1,
            wrong_value_error(
                req_type, route, len(course.get("students")
                                     ), 1, "length of students field"
            ),
        )
        for student in course.get("students"):
            ...
            self.assertEqual(
                ...
                None,
                wrong_value_error(
                    req_type, route, student.get(
                        "courses"), None, "type of courses field"
                ),
            )
        ...
        self.assertEqual(
            ...
            1,
            wrong_value_error(
                req_type, route, len(user.get("courses")
                                     ), 1, "length of courses list"
            ),
        )
        for course in user.get("courses"):
            ...
            self.assertEqual(
                ...
                None,
                wrong_value_error(
                    req_type, route, course.get(
                        "assignments"), None, "type of assignments field"
                ),
            )
            self.assertEqual(
                course.get("students"),
                ...
                wrong_value_error(
                    req_type, route, course.get(
                        "students"), None, "type of students field"
                ),
            )
            self.assertEqual(
                ...
                wrong_value_error(
                    req_type, route, course.get(
                        "instructors"), None, "type of instructors field"
                ),
            )
    ...
    def test_add_instructor_to_course(self):
        ...
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, type(course.get(
                    "students")), list, "type of instructors field"
            ),
        )
        self.assertEqual(
            ...
            1,
            wrong_value_error(
                req_type, route, len(course.get(
                    "instructors")), 1, "length of students list"
            ),
        )
        for instructor in course.get("instructors"):
            ...
            self.assertEqual(
                ...
                user_id,
                wrong_value_error(
                    req_type, route, instructor.get(
                        "id"), user_id, "instructor id"
                ),
            )
            self.assertEqual(
        ...
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, len(course.get(
                    "instructors")), 1, "length of instructors field"
            ),
        )
        for instructor in course.get("instructors"):
            for key in SAMPLE_USER:
    ...
    def test_create_assignment_for_course(self):
        ...
        for key in SAMPLE_ASSIGNMENT:
            self.assertEqual(
                ...
                wrong_value_error(
                    req_type, route, assignment.get(
                        key), SAMPLE_ASSIGNMENT[key], key, SAMPLE_ASSIGNMENT
                )
            )
        ...
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, len(course.get(
                    "assignments")), 1, "length of assignments list"
            ),
        )
        ...
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, assignment.get(
                    "course"), None, "assignment course field"
            )
        )
        self.assertEqual(
            assignment.get("id"),
    ...
    def test_extra_drop_course(self):
        ...
        self.assertEqual(
            ...
            0,
            wrong_value_error(
                req_type, route, len(student.get(
                    "courses")), 0, "length of courses filed"
            )
        )
    ...
    def text_extra_update_assignment(self):
        ...
        res = requests.post(gen_assignments_path(
            assignment_id), data=json.dumps(update_assignment_body))
        ...
        for key in update_assignment_body:
            self.assertEqual(
                ...
                wrong_value_error(
                    req_type, route, assignment.get(
                        key), update_assignment_body["key"], key
                )
            )
    ...
    def test_extra_update_invalid_assignment(self):
        ...
        update_assignment_body = {"title": "PA4 - CMS", "due_date": 1648093695}
        res = requests.post(gen_assignments_path(
            1000), data=json.dumps(update_assignment_body))
    ...
    def test_extra_submit(self):
        ...
        res = requests.post(gen_assignments_path(
            assignment_id) + "submit/", data=json.dumps(submission_body))
        ...
        self.assertEqual(
            ...
            wrong_value_error(
                req_type, route, submission.get(
                    "content"), submission_body["content"], "content"
            )
        )
    ...
    def test_extra_submit_invalid_assignment(self):
        ...
        submission_body = {"user_id": user_id, "content": "my assignment"}
        res = requests.post(gen_assignments_path(
            1000) + "submit/", data=json.dumps(submission_body))
    ...
    def test_extra_grade(self):
        ...
        res = requests.post(gen_assignments_path(
            assignment_id) + "grade/", data=json.dumps(grade_body))
    ...
    def test_grade_invalid_assignment(self):
        ...
        res = requests.post(gen_assignments_path(
            1000) + "grade/", data=json.dumps(grade_body))
    ...
    def test_grade_wrong_submission_assignment(self):
        ...
        submission_body = {"user_id": user_id, "content": "my assignment"}
        res = requests.post(gen_assignments_path(
            right_assignment_id) + "submit/", data=json.dumps(submission_body))
        submission_id = res.json()["id"]
        # grade + check response
        route = gen_assignments_route(wrong_assignment_id) + "grade/"
        grade_body = {"submission_id": submission_id, "score": 100}
        res = requests.post(gen_assignments_path(
            wrong_assignment_id) + "grade/", data=json.dumps(grade_body))
        self.jsonable_test(res, req_type, route, 404, grade_body)
        self.assertIsNotNone(res.json()["error"], wrong_value_error(
            req_type, route, res.json()["error"], str, "error", "type of error field"))
